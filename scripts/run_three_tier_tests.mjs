...
function compactSections(sections) {
  ...
  return {
    ...
    hotelTier: rowValue(overview, "酒店等级"),
    tierLabel: rowValue(overview, "方案档位"),
    transportMode: rowValue(overview, "交通方式"),
    guideType: rowValue(overview, "导游服务"),
    ...
    costsRows: costs.rows,
    costsClauses: costs.clauses || [],
...
JSON.stringify(cases.map((input) => {
  const plans = buildThreeTierPlans(input);
  const compacted = plans.map((plan) => ({
    tier: plan.tier,
    tierLabel: plan.profile.label,
    tagline: plan.profile.tagline,
    totalPrice: plan.totalPrice,
    featureRows: plan.featureRows,
    summary: compactSections(plan.sections),
    sections: plan.sections,
  }));
  const middlePlan = compacted.find((p) => p.tier === "舒适型") || compacted[1];
  const fullText = middlePlan.sections.map(renderTextSection).join("\\n\\n");
  const checks = {
    dayCountMatches: middlePlan.summary.dayCount === Number(input.days),
    denmarkNoIcelandLeak: !/丹麦/.test(input.destinations) || !/(冰岛|雷克雅未克|黄金圈|蓝湖|瓦特纳|钻石沙滩)/.test(fullText),
    finlandNoIcelandLeak: !/芬兰/.test(input.destinations) || !/(冰岛|雷克雅未克|瓦特纳|钻石沙滩)/.test(fullText),
    icelandNoFinlandLeak: !/冰岛/.test(input.destinations) || !/(芬兰|赫尔辛基|罗瓦涅米|萨利色尔卡)/.test(fullText),
    businessInviteOnlyWhenBusiness: /商务/.test(input.preferences) ? middlePlan.summary.invitationTitle === "商务邀请函" : middlePlan.summary.invitationTitle !== "商务邀请函",
  };
  return { input, plans: compacted, checks };
}), null, 2);
`;
...
const output = vm.runInNewContext(`${iconStubs}\n${logicSource}\n${runner}`, context, {
  filename: "AppThreeTierLogic.vm.js",
});
